...
module ram(
    input clk, input chipSelect, input reset, input outputEnable, 
    input writeEnable, input [3:0] address, input [15:0] writeData,  
    output [15:0] memOut
);
    ...
    decoder4to16 d1(address, decOut);
    // now we decode the address to know what regster we are pointing to using enable of register using enable
    ...
    // now we use the 32 to 1 mux 

    wire [15:0] tempMemOut;

    mux32to2 m1(Dmem0,   Dmem1,   Dmem2,   Dmem3,   Dmem4,   Dmem5,   Dmem6,   Dmem7,   
                Dmem8,   Dmem9,   Dmem10,  Dmem11,  Dmem12,  Dmem13,  Dmem14,  Dmem15,   
                Dmem16,  Dmem17,  Dmem18,  Dmem19,  Dmem20,  Dmem21,  Dmem22,  Dmem23, 
                Dmem24,  Dmem25,  Dmem26,  Dmem27,  Dmem28,  Dmem29,  Dmem30,  Dmem31, 
                address, tempMemOut);

    // we mux 16'b0 and tempMemOut with select line as outputEnable & chipSelect
    mux2to1_16bits m2(16'b0, tempMemOut, outputEnable & chipSelect, memOut);
    // if outputEnable = 0 or chipSelect = 0, memOut = 0, else it is tempMemOut
